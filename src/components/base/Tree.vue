...
<template>
  <div v-for="item of options" :key="item.value">
    <div class="flex items-center px-2 py-0.5 flex gap-1 transition-100 hover:bg-gray-100 hover:rounded">
      <span class="w-4 h-4 transition-100 cursor-pointer text-gray-500" :class="{
      ...
      <Checkbox v-if="selectable" :value="item.value" v-model="checkedValues" :disabled="item.disabled"
        @change="handleCheck">
        {{ item.label }}
      </Checkbox>
      <span v-else>
    ...
    <!-- secondary level -->
    <div v-for="sub of item.children" :key="sub.value" class="ml-10" :class="{
      'hidden': item.hidden,
    }">
      <div class="flex items-center px-2 py-0.5 flex gap-1 cursor-pointer transition-100 hover:bg-gray-100 hover:rounded">
        <Checkbox v-if="selectable" :value="sub.value" v-model="checkedValues" :disabled="sub.disabled"
          @change="handleCheck">
          {{ sub.label }}
        </Checkbox>
        <span v-else>
